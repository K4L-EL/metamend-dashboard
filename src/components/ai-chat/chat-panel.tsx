...
export function AiChatToggle() {
  ...
  return (
    <button
      onClick={toggle}
      title={open ? "Close MetaMed AI" : "Open MetaMed AI"}
      className={cn(
        "relative rounded-lg p-2 transition-colors",
        open
          ? "bg-sky-50 text-sky-600"
          : "text-neutral-500 hover:bg-neutral-100 hover:text-sky-600",
      )}
    >
      {open ? (
...
export function AiChatDrawer() {
  ...
  return (
    <div className="fixed right-4 top-[60px] z-40 flex h-[calc(100vh-72px)] max-h-[620px] w-[400px] flex-col rounded-2xl border border-neutral-200 bg-white shadow-2xl sm:right-6">
        ...
        {messages.length === 0 ? (
          ...
          <>
            {messages.map((msg) => (
              ...
              >
                {msg.role === "assistant" ? (
                  <Prose text={msg.content} variant="compact" />
                ) : (
                  msg.content
                )}
              </div>
