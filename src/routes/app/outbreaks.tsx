...
function generateReport(outbreak: Outbreak): string {
  const lines = [
    ...
    `INVESTIGATION: ${outbreak.investigationStatus}`,
    ``,
    `--- EXECUTIVE SUMMARY ---`,
    ``,
    ...
    ``,
    `Index Case: Patient admitted to ${outbreak.location} with ${outbreak.organism} ` +
    `${outbreak.organism === "MRSA" ? "bloodstream infection" : outbreak.organism === "C. difficile" ? "gastrointestinal infection" : "infection"} ` +
    `confirmed by laboratory culture on ${formatDate(outbreak.detectedAt)}.`,
    ``,
    `Transmission Pattern: Initial analysis suggests ${outbreak.affectedPatients > 3 ? "sustained" : "limited"} ` +
    `transmission within the ${outbreak.location} environment. Contact tracing identified ` +
    `${outbreak.affectedPatients + Math.floor(Math.random() * 3)} close contacts requiring screening.`,
    ``,
    ...
    `--- RECOMMENDATIONS ---`,
    ``,
    `1. Continue enhanced surveillance for additional 14 days post last case`,
